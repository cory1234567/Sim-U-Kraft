...
import info.satscape.simukraft.common.BlockMarker;
...
public class GuiControlBox extends GuiScreen
{
    ...
    public void drawScreen(int i, int j, float f)
    {
        try
        {
            ...
            drawCenteredString(fontRenderer, "Building Control Panel", width / 2, 17, 0xffffff);
            ...
            if (theBuilding != null)
            {
                ...
                fontRenderer.drawString("Building : " + theBuilding.displayNameWithoutPK + author, 5, 37, 0xffff80);
                fontRenderer.drawString("Type : " + theBuilding.type + " (" + isComplete + ")", 5, 47, 0xffff80);
                ...
                else if (theBuilding.type.contentEquals("industrial") || theBuilding.type.contentEquals("commercial"))
                {
                    ...
                    for (int fc = 0; fc < ModSimukraft.theFolks.size(); fc++)
                    {
                        ...
                        if (theBuilding.primaryXYZ.isSameCoordsAs(folk.employedAt, true, true))
                        {
                            fontRenderer.drawString(folk.name + " (" + folk.age + ") - "
            ...
            {
                fontRenderer.drawString("ERROR: lost info on this building (" + this.location.toString() + ")", 5, 77, 0xff0000);
                fontRenderer.drawString("only click 'fix house' below if this Building IS a house", 5, 97, 0xff0000);
            }
    ...
    public void actionPerformed(GuiButton guibutton)
    {
        ...
        else if (guibutton.displayString.contentEquals("Demolish!"))
        {
            ...
            {
                int l = theWorld.getBlockId(blockLoc.x.intValue(), blockLoc.y.intValue(), blockLoc.z.intValue());

                if (l > 0 && ModSimukraft.demolishBlocks.size() < 500)
                {
                    blockLoc.blockID = l;
                    ModSimukraft.demolishBlocks.add(blockLoc);
                }

                theWorld.setBlock(blockLoc.x.intValue(), blockLoc.y.intValue(), blockLoc.z.intValue(), 0, 0, 0x03);
    ...
    private void rotateStairs()
    {
        ...
        for (V3 blockLoc : theBuilding.blockLocations)
        {
            ItemStack is = new ItemStack(theWorld.getBlockId(blockLoc.x.intValue(), blockLoc.y.intValue(), blockLoc.z.intValue()), 1,
                                         theWorld.getBlockMetadata(blockLoc.x.intValue(), blockLoc.y.intValue(), blockLoc.z.intValue()));

            if (BlockStairs.isBlockStairsID(is.itemID)
            ...
            else if (is.itemID==Block.torchWood.blockID || is.itemID==Block.torchRedstoneActive.blockID || is.itemID==Block.torchRedstoneIdle.blockID) {
            ...
            // beds
            } else if(is.itemID==Block.bed.blockID) {
            	int newmeta = is.getItemDamage();
            	newmeta++;
            ...
            //pistons	
            } else if(is.itemID==Block.pistonBase.blockID || is.itemID==Block.pistonExtension.blockID
            	|| is.itemID==Block.pistonStickyBase.blockID) {
            	int newmeta = is.getItemDamage();
            ...
            } else if(is.itemID==Block.signPost.blockID) {
            	int newmeta = is.getItemDamage();
            ...
            } else if(is.itemID==Block.signWall.blockID || is.itemID==Block.ladder.blockID) {
            	int newmeta = is.getItemDamage();
            	if (newmeta==2) {newmeta=5;}
            	else if(newmeta==5) {newmeta=3;}
            	else if(newmeta==3) {newmeta=4;}
            ...
            //buttons
            } else if(is.itemID==Block.stoneButton.blockID || is.itemID==Block.woodenButton.blockID) {
            	int newmeta = is.getItemDamage();
            ...
            // fence gates
            } else if(is.itemID==Block.fenceGate.blockID) {
            	int newmeta = is.getItemDamage();
            	newmeta++;
