...
import info.satscape.simukraft.common.PacketHandler;
...
public class GuiBuildingConstructor extends GuiScreen
{
    ...
    public void drawScreen(int i, int j, float f)
    {
        try
        {
            ...
            drawCenteredString(fontRenderer, "Building Constructor", width / 2, 17, 0xffffff);
            String s = "Idle";
            ...
            drawCenteredString(fontRenderer, "Current status: " + s , width / 2, 30, 0xaaffff);
            drawCenteredString(fontRenderer, "Building type: " + t , width / 2, 40, 0xaaffff);

            if (currentPage == 0)
            {
                drawCenteredString(fontRenderer, "Please choose a task for this building constructor", width / 2, 100, 0xffffaa);
            }
            else if (currentPage == 1)
            {
                drawCenteredString(fontRenderer, "Please choose a type of building", width / 2, 100, 0xffffaa);
            }
            else if (currentPage == 2)
            {
                drawCenteredString(fontRenderer, "Now choose the residential building to build", width / 2, 50, 0xffffaa);
                tfSearch.drawTextBox();
            }
            else if (currentPage == 3)
            {
                drawCenteredString(fontRenderer, "Choose an unemployed Sim-U-Folk you want to hire", width / 2, 50, 0xffffaa);
            }
            else if (currentPage == 4)
            {
                drawCenteredString(fontRenderer, "Here are all your employees", width / 2, 50, 0xffffaa);
            }
            else if (currentPage == 5)
            {
                drawCenteredString(fontRenderer, "Now choose the commercial building to build", width / 2, 50, 0xffffaa);
                tfSearch.drawTextBox();
            }
            else if (currentPage == 6)
            {
                drawCenteredString(fontRenderer, "Now choose the industrial building to build", width / 2, 50, 0xffffaa);
                tfSearch.drawTextBox();
            }
            else if (currentPage == 7)
            {
                drawCenteredString(fontRenderer, "Now choose the other type of building to build", width / 2, 50, 0xffffaa);
                tfSearch.drawTextBox();
            }
            else if (currentPage == 8)
            {
                drawCenteredString(fontRenderer, "Building requirements for " + selectedBuilding.displayNameWithoutPK, width / 2, 50, 0xffffaa);
                int y = 70;
                ...
                while (it.hasNext())
                {
                    ...
                    if (is.itemID > 0)
                    {
                        if (y + 20 > (height - 20))
                        {
                            drawString(fontRenderer, "...and several more block types", 90, y, 0xffffff);
                        }
                        else
                        {
    ...
    private void displayReq(String block, int qty, int y)
    {
        ...
        drawString(fontRenderer, qty + "", 90, y, 0xffffff);
        drawString(fontRenderer, "x", 125, y, 0xffffff);
        drawString(fontRenderer, block, 150, y, 0xffffff);
        String st = "";
        ...
        drawString(fontRenderer, st, 250, y, 0xffffff);
    }
    ...
    private void showPage()
    {
        ...
        else if (currentPage == 2 || currentPage == 5 || currentPage == 6 || currentPage == 7) //what r/c/i/o building
        {
            ...
            String theType = "";
            buildingsOnPage = 0;
            tfSearch = new GuiTextField(fontRenderer, width / 2 - 50, height - 30, 100, 20);
    ...
    public void actionPerformed(GuiButton guibutton)
    {
        ...
        else if (currentPage == 8)   // building requirements (what blocks are need)
        {
            if (guibutton.displayString.contentEquals("Build it!"))
            {
                ...
                PacketDispatcher.sendPacketToServer(PacketHandler.makePacket("","loadbuilding","GuiBuildingCon"));
