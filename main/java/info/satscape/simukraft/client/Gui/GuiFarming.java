...
public class GuiFarming extends GuiScreen
{
    ...
    public void drawScreen(int i, int j, float f)
    {
        ...
        try
        {
            ...
            {
                drawCenteredString(fontRenderer, "ERROR WITH FARMING BOX, Place 3 markers, then place Farming box", width / 2, 17, 0xffffff);
                return;
            }

            drawCenteredString(fontRenderer, "Level " + theFarmingBox.level + " " + theFarmingBox.farmType.toString() +
            ...
            try
            {
                ...
                {
                    drawCenteredString(fontRenderer, "Error: No markers placed - 3 markers are needed to farm an area.",
                                       width / 2, 27, 0xff0000);
                }
            }
            catch (Exception e)
            {
                drawCenteredString(fontRenderer, "Error: Place markers BEFORE the farming box.",
                                   width / 2, 27, 0xff0000);
            }

            if (theFarmingBox.level == 1 && theFarmingBox.farmType == FarmType.SUGAR)
            {
                drawCenteredString(fontRenderer, "Cannot upgrade sugar cane farms above level 1",
                                   width / 2, 130, 0xffffff);
            }
            else if (theFarmingBox.level == 1 && theFarmingBox.farmType == FarmType.CACTUS)
            {
                drawCenteredString(fontRenderer, "Cannot upgrade cactus farms above level 1",
                                   width / 2, 130, 0xffffff);
            }
            else if (theFarmingBox.level < 3)
            {
                ...
                {
                    drawCenteredString(fontRenderer, "Upgrade will cost " + ModSimukraft.displayMoney(getUpgradeCost()) + " credits",
                                       width / 2, 130, 0xffffff);
                }
                else
                {
                    drawCenteredString(fontRenderer, "Upgrade is Free",
                                       width / 2, 130, 0xffffff);
                }
            }
