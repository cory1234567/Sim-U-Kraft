...
public class GuiMarker extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        else if (BlockMarker.markers.size() == 1)
        {
            ...
            theGuiTextField1 = new GuiTextField(fontRenderer, width / 2 - (width / 3 / 2), 138, width / 3, 20);
    ...
    public void drawScreen(int i, int j, float f)
    {
        ...
        drawCenteredString(fontRenderer, "Sim-u-Markers", width / 2, 30, 0xffffff);
        drawCenteredString(fontRenderer, "Markers can be used to make a copy of a building or you can use them to", width / 2, 40, 0xa0ffff);
        drawCenteredString(fontRenderer, "mark out a mining or food-based farming area.", width / 2, 55, 0xa0ffff);
        drawCenteredString(fontRenderer, "A single marker can be used to designate a new lumberjack area and more!", width / 2, 70, 0xa0ffff);
        drawCenteredString(fontRenderer, "TIP: remove old markers after use, before marking a new area.", width / 2, 85, 0xa0ffff);
        drawCenteredString(fontRenderer, errorText, width / 2, height - 50, 0xff0000);
                ...
                for (int l = 0; l < 200; l++)
                {
                    ...
                    for (int ftb = 0; ftb < ftbCount; ftb++)
                    {
                        for (int ltr = 1; ltr <= ltrCount; ltr++)
                        {
                            ...
                            int yyy = (by + l) - 1;
                            int zzz = bz + zo;
                            iD = mc.getIntegratedServer().worldServerForDimension(thePlayer.dimension).getBlockId(xxx, yyy, zzz);
