...
public class GuiCourierTasks extends GuiScreen
{
    ...
    public void drawScreen(int i, int j, float f)
    {
        ...
        if (onPage.contentEquals("main"))
        {
            drawCenteredString(fontRenderer, "Courier tasks for " + theFolk.name, width / 2, 17, 0xffffff);
            int idx = 2;
            int y;
            ...
            for (int t = 0; t < ModSimukraft.theCourierTasks.size(); t++)
            {
                ...
                if (ct.folkname.contentEquals(theFolk.name))
                {
                    ...
                    {
                        fontRenderer.drawString(ct.pickup.name, 5, y - 5, 0xffff8f);
                        fontRenderer.drawString("->", width / 3, y - 5, 0xffff00);

                        if (ct.dropoff == null)
                        {
                            fontRenderer.drawString("The Depot", width / 2, y - 5, 0xf0ff8f);
                        }
                        else
                        {
                            fontRenderer.drawString(ct.dropoff.name, width / 2, y - 5, 0xf0ff8f);
                        }
        ...
        else if (onPage.contentEquals("add"))
        {
            ...
            {
                drawCenteredString(fontRenderer, "Choose a pick up point", width / 2, 17, 0xffffff);
            }
            else
            {
                drawCenteredString(fontRenderer, "Pick up from " + newtask.pickup.name + " and drop off at...", width / 2, 17, 0xffffff);
            }
        }
