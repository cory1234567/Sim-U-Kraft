...
public class JobBaker extends Job implements Serializable
{
    ...
    private void stageCollectingWheat()
    {
        ...
        {
            farmChests = inventoriesFindClosest(farm.getLocation(), 5);
            inventoriesTransferToFolk(theFolk.inventory, farmChests, new ItemStack(Item.wheat, 640),-1);
            step = 3;
        }
        else if (step == 3)
        {
    ...
    private void stageMakeBread()
    {
        ...
        {
            int wheat = getInventoryCount(theFolk, Item.wheat.itemID);
            ...
            try
            {
                bakeryChests.get(0).openChest();
            }
            catch (Exception e) {}

            inventoriesPut(bakeryChests, new ItemStack(Item.bread, bread), true);
            theFolk.inventory.clear();
            step = 3;
        }
        else if (step == 3)
        {
            bakeryChests.get(0).closeChest();
            ...
            if (theFolk.theEntity != null)
            {
                if (theFolk.gender == 0)
                {
                    mc.theWorld.playSound(theFolk.location.x, theFolk.location.y, theFolk.location.z, "satscapesimukraft:bakerm", 1f, 1f, false);
                }
                else
                {
                    mc.theWorld.playSound(theFolk.location.x, theFolk.location.y, theFolk.location.z, "satscapesimukraft:bakerf", 1f, 1f, false);
                }
            }
    ...
    private void stageSellingBread()
    {
        if (step == 1)
        {
            if (pay > 0)
            {
                ...
                mc.theWorld.playSound(mc.thePlayer.posX,
                                      mc.thePlayer.posY, mc.thePlayer.posZ, "satscapesimukraft:cash", 1f,
                                      1f, false);
            }
        ...
        {
            if (ModLoader.getMinecraftServerInstance().worldServers[0].getWorldTime() % 24000 > 11600)
            {
                step = 3;
            }
        }
        else if (step == 3)
        {
            ...
            if (ModSimukraft.theFolks.size() > 1)
            {
                ...
                bakeryChests = inventoriesFindClosest(theFolk.employedAt, 4);
                breadStack = inventoriesGet(bakeryChests, new ItemStack(Item.bread, sell), false,false,-1);
            }
