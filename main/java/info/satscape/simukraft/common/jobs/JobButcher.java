...
public class JobButcher extends Job implements Serializable
{
    ...
    private void stageCollectingMeat()
    {
        ...
        {
            inventoriesTransferToFolk(theFolk.inventory, chestsAtFarm, new ItemStack(Item.chickenRaw, 640),-1);
            inventoriesTransferToFolk(theFolk.inventory, chestsAtFarm, new ItemStack(Item.porkRaw, 640),-1);
            inventoriesTransferToFolk(theFolk.inventory, chestsAtFarm, new ItemStack(Item.beefRaw, 640),-1);
            step = 3;
        }
        else if (step == 3)
        {
    ...
    private void stageGoBackToStore()
    {
        ...
        else
        {
            ...
            if (dist < 2)
            {
                ...
                int meat1 = getInventoryCount(theFolk, Item.porkRaw.itemID);
                int meat2 = getInventoryCount(theFolk, Item.chickenRaw.itemID);
                int meat3 = getInventoryCount(theFolk, Item.beefRaw.itemID);
    ...
    private void stageSellingMeat()
    {
        ...
        else if (step == 3)
        {
            ...
            {
                piece = inventoriesGet(chestsAtShop, new ItemStack(Item.porkRaw, 1), false,false,-1);

                if (piece == null)
                {
                    piece = inventoriesGet(chestsAtShop, new ItemStack(Item.chickenRaw, 1), false,false,-1);
                }

                if (piece == null)
                {
                    piece = inventoriesGet(chestsAtShop, new ItemStack(Item.beefRaw, 1), false,false,-1);
                }
