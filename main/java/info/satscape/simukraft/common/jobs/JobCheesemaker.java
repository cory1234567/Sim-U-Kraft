...
public class JobCheesemaker extends Job {
    ...
    private void openCloseMilkShutters(boolean open) {
    	...
    	if (open) {
    		...
    		newId=Block.blockIron.blockID;
    	}
    	
    	int current=jobWorld.getBlockId(shutters.get(0).x.intValue(),shutters.get(0).y.intValue(),shutters.get(0).z.intValue());
    	
    	if (current !=newId) {
    		if (open && current==ModSimukraft.blockFluidMilk.blockID) { return; } //opening and it already has milk flowing
     		
			for(V3 v3:shutters) {
				jobWorld.setBlock(v3.x.intValue(),v3.y.intValue(),v3.z.intValue(),newId,0,0x03);
			}
	...
	private void stageArrivedAtFactory() {
		...
		inventoriesTransferToFolk(theFolk.inventory, chests, new ItemStack(Item.bucketMilk,64),-1);
		} catch(Exception e) {}
	...
	private void stageCollectingMilk() {
		...
		if (step==1) {
			...
			inventoriesTransferToFolk(theFolk.inventory, chestsAtDairy, new ItemStack(Item.bucketMilk,1),-1); 
	...
	private void stageEmptyingMilk() {
		if (step==1) {
			...
			for(V3 milkBlock:milkblocks) {
				int id=jobWorld.getBlockId(milkBlock.x.intValue(), milkBlock.y.intValue(),milkBlock.z.intValue());
				int meta=jobWorld.getBlockMetadata(milkBlock.x.intValue(), milkBlock.y.intValue(),milkBlock.z.intValue());
				if (id==0 || (id ==ModSimukraft.blockFluidMilk.blockID && meta ==1)) {
					jobWorld.setBlock(milkBlock.x.intValue(), milkBlock.y.intValue(), milkBlock.z.intValue()
							, ModSimukraft.blockFluidMilk.blockID, 0, 0x03);
	...
	private void transformMilkToCheese(V3 currentStirPos) {
		...
		for(int m=milkBlocks.size()-1;m>0;m--) {
			...
			int id=jobWorld.getBlockId(milk.x.intValue(), milk.y.intValue(), milk.z.intValue());
			int meta=jobWorld.getBlockMetadata(milk.x.intValue(), milk.y.intValue(), milk.z.intValue());
			if (id==ModSimukraft.blockFluidMilk.blockID && meta==0) {
				jobWorld.setBlock(milk.x.intValue(), milk.y.intValue(), milk.z.intValue(),0,0,0x03);
				milkGotCount++;
		...
		if (milkGotCount>0) {
			for(V3 cheese:cheeseBlocks) {
				int id=jobWorld.getBlockId(cheese.x.intValue(), cheese.y.intValue(), cheese.z.intValue());
				int dist=cheese.getDistanceTo(currentStirPos);
				if (id != ModSimukraft.blockCheese.blockID && dist<5) {
					jobWorld.setBlock(cheese.x.intValue(), cheese.y.intValue(), cheese.z.intValue()
							,ModSimukraft.blockCheese.blockID,0,0x03);
					placedCheese=true;
					break;
				}
			}
	...
	private void stageHarvestCheese() {
		...
		if (step==1) {
			...
			for(V3 block:cheeseBlocks) {
				int id=jobWorld.getBlockId(block.x.intValue(),block.y.intValue(),block.z.intValue());
				if(stirPositions.get(0).getDistanceTo(block) <5 && id==ModSimukraft.blockCheese.blockID) {
					gotBlock=true;
					theFolk.inventory.add(new ItemStack(ModSimukraft.blockCheese));
					jobWorld.setBlock(block.x.intValue(),block.y.intValue(),block.z.intValue(),0,0,0x03);
					theFolk.isWorking=true;
			...
			for(V3 block:cheeseBlocks) {
				int id=jobWorld.getBlockId(block.x.intValue(),block.y.intValue(),block.z.intValue());
				if(stirPositions.get(1).getDistanceTo(block) <5 && id==ModSimukraft.blockCheese.blockID) {
					gotBlock=true;
					theFolk.inventory.add(new ItemStack(ModSimukraft.blockCheese));
					jobWorld.setBlock(block.x.intValue(),block.y.intValue(),block.z.intValue(),0,0,0x03);
					break;
