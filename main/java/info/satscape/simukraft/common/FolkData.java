...
public class FolkData implements Serializable
{
   ...
   // public String partneredWith = "";
    ...
    public void hasLoaded()
    {
        ...
        respawnEntity(ModLoader.getMinecraftServerInstance().worldServerForDimension(this.location.theDimension));
    ...
    public void respawnEntity(World world)
    {
        ...
        if (getDistanceToPlayer() < 50)
        {
            ...
            entityId = this.theEntity.entityId;
            ModSimukraft.log.info("FolkData:repawnEntity() " + this.name + " at " + this.location.toString() + " in dim " + this.location.theDimension + " ENTITY:" + this.theEntity.entityId);
        }
    }
    ...
    public void onUpdate()
    {
        ...
        if (now - timeSinceLastMinute > 60000)
        {
            ...
            if (t<2000 && this.pregnancyStage>=1.0f) {
                ...
                new FolkData(ModLoader.getMinecraftServerInstance().worldServerForDimension(0), this, male);
            }
            ...
            if (action == FolkAction.WANDER && this.isSpawned() && this.employedAt == null && this.age >= 18 && !this.statusText.contains("baby"))
            {
                ...
                if (!gotWanderPoint)
                {
                    ...
                    World world = ModLoader.getMinecraftServerInstance().worldServerForDimension(location.theDimension);

                    while (world.getBlockId(wanderTo.x.intValue(), wanderTo.y.intValue(), wanderTo.z.intValue()) > 0
                            && wanderTo.y < 255)
                    {
                        wanderTo.y++;
                    }
        ...
        if (now - timeSinceLastStatusUpdate > 1000)
        {
            ...
            if (!isSpawned())
            {
                ...
                if (range < 50)
                {
                    respawnEntity(ModLoader.getMinecraftServerInstance()
    ...
    public V3 getLocationCloseToPlayer()
    {
        ...
        try
        {
            for (int go = 30; go > 1; go--)
            {
                ...
                while (!found)
                {
                    bid = p.worldObj.getBlockId(ret.x.intValue(), ret.y.intValue(), ret.z.intValue()); // this can NPE

                    if ((p.worldObj.canBlockSeeTheSky(ret.x.intValue(), ret.y.intValue(), ret.z.intValue()) ||
                            p.dimension != 0) &&
                            bid != Block.leaves.blockID &&
                            bid == 0)
                    {
                        found = true;
                    }
            ...
            for (int inv = 0; inv < inventory.size(); inv++)
            {
                ...
                if (is != null)
                {
                    if (theEntity != null)
                    {
                        try
                        {
                            theEntity.dropItem(is.itemID, is.stackSize);
                        }
                        catch (Exception e) {}
                    }
                    else
                    {
                        try
                        {
                            FolkData.getClosestPlayer(this.location).dropItem(is.itemID, is.stackSize);
                        }
                        catch (Exception e) {}
                    }
    ...
    public void gotoXYZ(V3 whereTo, GotoMethod methodOfTravel)
    {
        ...
        if (methodOfTravel == null)
        {
            ...
            if (playpos != null)  //will be null if player in different dimension or WAY out of range
            {
                ...
                try
                {
                    if (location.theDimension != ModLoader.getMinecraftInstance().thePlayer.dimension &&
                            this.destination.theDimension != ModLoader.getMinecraftInstance().thePlayer.dimension)
                    {
                        this.gotoMethod = GotoMethod.SHIFT;
                    }
                }
    ...
    public void beamMeTo(V3 whereToIn)
    {
        ...
        World destWorld = ModLoader.getMinecraftServerInstance().worldServerForDimension(whereTo.theDimension);

        for (int y = 0; y < 200; y++)
        {
            int id1 = destWorld.getBlockId(whereTo.x.intValue(), whereTo.y.intValue(), whereTo.z.intValue());
            int id2 = destWorld.getBlockId(whereTo.x.intValue(), whereTo.y.intValue() + 1, whereTo.z.intValue());
    ...
    private void doBeaming()
    {
        try
        {
            if (System.currentTimeMillis() - timeStartedGotoing > 4000 || beamingTo == null)
            {
                ...
                this.destination = null;
                beamingTo = null;
                respawnEntity(ModLoader.getMinecraftServerInstance().worldServerForDimension(location.theDimension));
                return;
            }
        }
        catch (Exception e)
        ...
        this.stayPut = true;
        World theWorld = ModLoader.getMinecraftInstance().theWorld; //only need the client world for particles
